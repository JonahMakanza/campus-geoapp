...
export default function Layout() {
    ...
    // YOUR NGROK URL - REPLACE THIS WITH YOUR ACTUAL NGROK URL
    const DEFAULT_NGROK_URL = 'https://stitch-murky-brush.ngrok-free.dev';

    useEffect(() => {
        networkManager.initialize(DEFAULT_NGROK_URL).then(async (url) => {
    ...
    const handleServerSettings = () => {
        Alert.prompt(
            'Server URL',
            'Enter the ngrok URL (e.g., https://your-url.ngrok.io)\n\nCurrent: ' + (serverUrl || 'Not set'),
            [
                    ...
                    onPress: async (url) => {
                        if (url && url.trim()) {
                            ...
                            if (success) {
                                ...
                                Alert.alert('Error', 'Could not connect to the server. Check:\n1. Ngrok is running\n2. URL is correct\n3. Server is started');
                            }
                        }
                    }
                }
    ...
    return (
        <>
            ...
            >
                <Stack.Screen name="index" options={{ title: 'UZ Campus GeoApp', headerShown: true }} />
