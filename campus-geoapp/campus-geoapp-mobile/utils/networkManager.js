...
class NetworkManager {
    ...
    async initialize(defaultUrl = null) {
        ...
        this.currentServerUrl = savedUrl || defaultUrl || 'http://192.168.137.88:3001';
    ...
    async addToOfflineQueue(data) {
        const queue = await this.getOfflineQueue();
        queue.push({ id: Date.now(), data: data, savedAt: new Date().toISOString() });
    ...
    getServerUrl() { return this.currentServerUrl; }
    ...
    removeListener(callback) { const index = this.listeners.indexOf(callback); if (index > -1) this.listeners.splice(index, 1); }
    notifyListeners(event, data) { this.listeners.forEach(l => { try { l(event, data); } catch {} }); }
}
